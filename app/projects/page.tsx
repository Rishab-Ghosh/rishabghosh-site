        ...
        <div className="mb-20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
            {projects.map((project, index) => (
              <div 
                key={index}
                className="group bg-surface/50 rounded-lg p-6 border border-primary/20 hover:border-primary/60 transition-all duration-300 hover:scale-105"
              >
                {/* Project Header */}
                <div className="mb-4">
                  <h3 className="text-xl font-space font-semibold text-primary mb-2 group-hover:neon-glow transition-all duration-300">
                    {project.title}
                  </h3>
                ...
                <div className="mb-6">
                  <div className="flex flex-wrap gap-2">
                    {project.techStack.map((tech, techIndex) => (
                      <span
                        key={techIndex}
                        className="px-3 py-1 bg-primary/20 text-primary text-xs font-inter font-medium rounded-full border border-primary/30"
                      >
                        {tech}
                      </span>
                    ))}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex space-x-3">
                    <a
                      ...
                      className="inline-flex items-center px-3 py-2 text-sm font-inter font-medium text-highlight/80 bg-accent rounded-md hover:bg-primary/20 hover:text-primary transition-all duration-200"
                    >
                    ...
                    {project.demoUrl && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center px-3 py-2 text-sm font-inter font-medium text-highlight bg-primary rounded-md hover:bg-primary/80 transition-all duration-200"
                      >
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {comingSoon.map((project, index) => (
              <div 
                key={index}
                className="bg-surface/30 rounded-lg p-6 border-2 border-dashed border-primary/30 hover:border-primary/50 transition-all duration-300"
              >
